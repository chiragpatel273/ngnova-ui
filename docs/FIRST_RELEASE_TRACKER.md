...
| Documentation initial bundle budget              | Ready       | `src/app/app.routes.ts`, `src/app/app.spec.ts`, `npm.cmd run build:demo:app`        | Lazy screen boundaries reduced the production initial bundle from 624.67 kB to 333.89 kB while retaining the 500 kB warning budget.              |
